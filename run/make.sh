...
sjasmplus -DSAVE_BIN=1 -DCOLLECT_INFO=0 --color=on -Wno-rdlow --msg=war --nologo --syntax=f --fullpath --lst="$repo_root/Build/Prebuilds.LST" "$repo_root/KODE_BIN.asm"
"$mhmt_bin" -hst -zxh "$repo_root/build/bin/Prepare.bin" "$repo_root/build/bin/HRUST/Prepare.hst"
"$mhmt_bin" -hst -zxh "$repo_root/build/bin/KodeMain.bin" "$repo_root/build/bin/HRUST/KodeMain.hst"
...
sjasmplus --longptr --nologo --syntax=f --fullpath --lst="$repo_root/Build/KodeEXE.LST" "$repo_root/KodeEXE.asm"
